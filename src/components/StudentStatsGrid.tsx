...
const StudentStatsGrid = ({ user }: StudentStatsGridProps) => {
  ...
  return (
    <>
      ...
      <div id="stats-grid" className="grid grid-cols-2 lg:grid-cols-2 gap-3 lg:gap-3">
        {stats.map((stat, i) => (
          <div key={i} className={`relative overflow-hidden rounded-xl lg:rounded-2xl stats-glass-card p-3 lg:p-4 flex flex-row items-center gap-3 transition-all duration-300 hover:shadow-lg h-[65px] lg:h-[120px] group`}>
            
            <div className={`w-9 h-9 lg:w-11 lg:h-11 rounded-xl ${stat.iconBg} flex items-center justify-center ${stat.color} shrink-0 shadow-sm border border-white/50`}>
              <stat.icon className="w-5 h-5" />
            </div>
            
            <div className="flex flex-col flex-1 min-w-0">
              <div className="flex items-center justify-between w-full mb-1">
                <p className="stat-label-mini">{stat.label}</p>
                <span className={`insight-tag-mini ${stat.color} hidden sm:inline-block`}>
                  {stat.insight}
                </span>
              </div>
              <p className="stat-value-sharp truncate">{stat.value}</p>
            </div>

            {/* Decorative background accent */}
            <div className={`absolute -right-2 -bottom-2 w-10 lg:w-16 h-10 lg:h-16 rounded-full blur-2xl opacity-10 group-hover:opacity-20 transition-opacity ${stat.bg}`} />
          </div>
        ))}
      </div>
    </>
