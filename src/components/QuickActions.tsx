                  ...
                  {getFilteredBulkStudents().length === 0 ? (
                    ...
                    getFilteredBulkStudents().map(s => (
                      <div 
                        ...
                        className={`flex items-center justify-between px-4 py-3 border-b border-gray-50 last:border-0 transition-colors cursor-pointer ${
                          selectedIds.has(s.uid || s.id) ? 'bg-blue-50/50' : 'hover:bg-gray-50'
                        }`}
                      >
                        <div className="flex items-center gap-3">
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center font-black text-[10px] transition-colors overflow-hidden border ${
                            selectedIds.has(s.uid || s.id) ? 'bg-blue-500 text-white border-blue-400' : 'bg-blue-100 text-blue-600 border-blue-50'
                          }`}>
                            {s.photoURL ? (
                              <img 
                                src={s.photoURL} 
                        ...
                        <div className={`w-5 h-5 rounded border transition-all flex items-center justify-center ${
                          selectedIds.has(s.uid || s.id) ? 'bg-blue-500 border-blue-500' : 'bg-white border-gray-200'
                        }`}>
