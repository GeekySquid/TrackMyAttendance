...
import { Users, UserPlus, UserCheck, UserMinus, Plus, Search, Copy, XCircle, Loader2 } from 'lucide-react';
    ...
    <div className="flex-1 overflow-y-auto p-4 sm:p-8 relative">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-8">
        {isLoading ? (
          ...
          <>
            <StatCard title="Total Students" value={students.length.toString()} total="" percentage="100%" trend="+12" trendUp={true}
              icon={<Users className="h-6 w-6 text-blue-600" />} colorClass="text-blue-600" bgClass="bg-blue-50" progressColorClass="bg-blue-500" />
            <StatCard title="New Enrollments (30d)" value={newEnrollments.toString()} total="" percentage={`${students.length > 0 ? Math.round((newEnrollments/students.length)*100) : 0}%`} trend="+5%" trendUp={true}
            ...
            <StatCard title="Active Students" value={activeStudents.toString()} total={students.length.toString()} percentage={`${students.length > 0 ? Math.round((activeStudents/students.length)*100) : 0}%`} trend="-2%" trendUp={false}
            ...
            <StatCard title="Dropped Out" value={droppedStudents.toString()} total={students.length.toString()} percentage={`${students.length > 0 ? Math.round((droppedStudents/students.length)*100) : 0}%`} trend="-1%" trendUp={false}
